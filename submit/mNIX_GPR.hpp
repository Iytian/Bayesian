...
namespace losmix {
  ...
  private:
    ...
    Eigen::LLT<MatrixXd_t> lltxx_; // for covariance matrix                               //Iytian added a new variable for inverse covariance matrix
    // simulation
    matrix<Type> z_;
  public:
    ...
    void set_suff(cRefMatrix_t& y, cRefMatrix_t& Xtr, cRefMatrix_t& cov);                 //Iytian added a new input for covariance matrix
  ...
  template <class Type>
  inline mNIX<Type>::mNIX(int p, int n) {
    p_ = p; 
    N_ = n;                                                                             //Iytian added n - dimension for covariance matrix
    ...
    lltxx_.compute(utils<Type>::identity_matrix(N_, N_));                               //Iytian  memory allocation for covariance matrix            
    // simulation
  ...
  inline void mNIX<Type>::set_suff(cRefMatrix_t& y, 
                        cRefMatrix_t& Xtr, cRefMatrix_t& cov) {
    lltxx_.compute(cov);                                                                //Iytian added inverse of covariance matrix
    XX_ = Xtr * lltxx_.solve(Xtr.transpose()); // can't use .noalias()                  //Iytian         
    Xy_ = Xtr * lltxx_.solve(y);                                                        //Iytian    
    yy_ = utils<Type>::dot_product(y, lltxx_.solve(y));                                 //Iytian
    N_ = Xtr.cols();
    return;
  }
